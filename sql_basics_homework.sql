...
SELECT user_id, COUNT(*) AS comment_count FROM comments GROUP BY user_id ORDER BY comment_count DESC LIMIT 1;
...
SELECT followed_id AS user_id, COUNT(*) AS followers_count FROM followers GROUP BY followed_id ORDER BY followers_count DESC LIMIT 1;

-- Q8: Count users joined in Feb 2023 with 'john' in username
SELECT COUNT(*) FROM users WHERE join_date BETWEEN '2023-02-01' AND '2023-02-28' AND username ILIKE '%john%';

-- Q9: Count users joined in Mar 2023 with 'mar' in username
SELECT COUNT(*) FROM users WHERE join_date BETWEEN '2023-03-01' AND '2023-03-31' AND username ILIKE '%mar%';

-- Q10: Return post_id with the most reactions
SELECT post_id, COUNT(*) AS reaction_count FROM reactions GROUP BY post_id ORDER BY reaction_count DESC LIMIT 1;
